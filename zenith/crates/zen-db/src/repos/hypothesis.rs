...
impl ZenService {
    ...
    pub async fn create_hypothesis(
        ...
        self.db().conn().execute(
            "INSERT INTO hypotheses (id, research_id, finding_id, session_id, content, status, created_at, updated_at)
             VALUES (?1, ?2, ?3, ?4, ?5, 'unverified', ?6, ?6)",
            libsql::params![
                id.as_str(),
                research_id,
                finding_id,
    ...
    pub async fn update_hypothesis(
        ...
        let sql = format!(
            "UPDATE hypotheses SET {} WHERE id = ?{idx}",
            sets.join(", ")
        );
