...
- `git clone` + `znt rebuild` gives you the full project knowledge base on a new machine
...
3. [Resolved: Rebuild From JSONL](#3-resolved-rebuild-from-jsonl)
...
Every mutation is recorded as an **operation** in JSONL. The SQLite DB is a materialized view that can be rebuilt by replaying the JSONL. Recovery sequence: `rm zenith.db && znt rebuild`.
...
1. **`.gitignore` template** generated by `znt init`:
   ```
...
3. **Post-checkout hook** (Approach B only, `hooks/post-checkout`):
   ...
   - If so, triggers `znt rebuild` to reconstruct the DB
...
1. Starts its own Zenith session (`znt session start` → unique `ses-xxx` ID)
...
4. Add `znt rebuild` command to `04-cli-api-design.md`
5. Update `INDEX.md`
...
2. Implement `.gitignore` template in `znt init`
...
- Schema validation for trail operations: [12-schema-spike-plan.md](./12-schema-spike-plan.md)
- Trail versioning strategy: [14-trail-versioning-spike-plan.md](./14-trail-versioning-spike-plan.md) (Approach D: `v` field, additive evolution, migration dispatch)
- Git hooks spike: [11-git-hooks-spike-plan.md](./11-git-hooks-spike-plan.md) (pre-commit JSONL validation, post-checkout rebuild)
