...
3. [Design Questions (All Resolved)](#3-design-questions-all-resolved)
...
The current implementation plan has task 5.18 as a single line: "Implement `znt init` .gitignore template and pre-commit hook." But the JSONL strategy doc (`10-git-jsonl-strategy.md`) describes three hooks and multiple open questions remain unaddressed:
...
| Hook implementation language undecided | Shell scripts can't reliably validate JSON without `jq` (not installed by default on macOS or many Linux distros). Rust validation via `znt hook` is robust but requires binary in PATH. |
...
- Vector database rebuild from JSONL (same as Zenith's `znt rebuild`)
...
|---|----------|-------------------|
| 1 | Should hooks be shell scripts, Rust (`znt hook <name>`), or thin-shell-wrapping-Rust? | A (shell), B (Rust), C (thin wrapper) |
...
Hooks are bash scripts generated by `znt init`, stored in `.zenith/hooks/`.
...
### Approach B: `znt hook <name>` Subcommand
...
All validation logic lives in `znt hook pre-commit` (Rust):
...
**Cons**: Requires `znt` binary in PATH. If user hasn't installed `znt` globally (e.g., using `cargo run`), hooks fail.
...
**Note on `jq`**: Earlier designs considered falling back to `jq` or `python3` for JSON validation. Neither is reliably available: `jq` is not installed by default on macOS or many Linux distros; `python3` may not be present on minimal systems. The spike will confirm this limitation and document that **Rust validation via `znt hook` is the only reliable path**.
...
| List git-backed sessions | List tags matching `zenith/ses-*` | `znt session list` can show git-visible sessions |
...
| 13 | `spike_postcheckout_auto_rebuild` | When JSONL changed on branch switch, trigger `znt rebuild` (simulated — replay JSONL to fresh DB). **Measure wall time** for: 100 operations, 1000 operations, 5000 operations. Record: wall time (ms), rebuilt DB size, FTS5 search works after rebuild. This test produces the data for the auto-vs-warn decision. |
| 14 | `spike_postcheckout_warn_only` | Alternative: detect changes, print "Zenith: JSONL trail changed on branch switch. Run `znt rebuild` to update database." and exit 0. Measure: near-zero delay. Compare: UX of auto-rebuild (seamless but slow) vs warn (instant but requires manual action). |
...
| # | Test | What It Validates |
|---|------|-------------------|
| 22 | `spike_compare_all` | Print four comparison tables: **(1) Installation strategy**: `core.hooksPath` vs symlink vs chain — columns: coexistence with existing hooks, install complexity, uninstall complexity, version-controlled hooks, recommendation. **(2) Hook implementation**: shell vs Rust (`znt hook`) vs wrapper — columns: validation reliability (edge cases caught from tests 8-9), speed, testability, PATH dependency, recommendation. **(3) Post-checkout behavior**: auto-rebuild vs warn — columns: time at 100/1000/5000 ops (from test 13), UX impact, recommendation with threshold. **(4) `gix` verdict**: features used, dependency cost, alternatives (shell out to `git`), recommendation. |
...
| > 5s | Warn-only always. Auto-rebuild only via explicit `znt rebuild`. |
...
3. Update `04-cli-api-design.md`: `znt session start` could accept `--tag` flag or auto-tag
...
| `znt rebuild` is too slow for post-checkout hook | Branch switches become sluggish | Low (< 5K ops) | Spike measures at 100/1000/5000 ops. Threshold-based decision: auto below threshold, warn above. Configurable via `.zenith/config.toml`. |
| `znt` binary not in PATH when hooks run | Hooks fail or skip validation silently | Medium | Wrapper approach (Approach C): graceful fallback with guidance message. Pre-commit skips validation rather than blocking commit. |
...
- Schema validation (supersedes hand-written schema): [12-schema-spike-plan.md](./12-schema-spike-plan.md) (schemars-generated schemas replace spike 0.13 hand-written)
- Trail versioning: [14-trail-versioning-spike-plan.md](./14-trail-versioning-spike-plan.md) (envelope `v` field validated)
